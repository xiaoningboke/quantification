...
class Index extends Controller{
   ...
   public function grade(){
        ...
        $t = count($data);
        for($i=0;$i<$t;$i++){
            $s[$i]= "{"."value".":"."'".$data[$i]["ma_majorname"]."'".","."text".":"."'".$data[$i]["ma_majorname"]."'"."}";
        }
     ...
     public function addclasses()
   {
        ...
        $major = model('ClassesModel');
          $result = $major->addClasses($cl_grade,$cl_major,$cl_classes,$cl_headmaster,$cl_remarks);
        if ($result) {
             echo "操作成功";
           } else {
             echo "操作失败";
    ...
    */
    public function editclasses(){
        ...
        $cl_remarks = input('post.cl_remarks');
        $classes = model('ClassesModel');
          $result = $classes->editclasses($id,$cl_grade,$cl_major,$cl_classes,$cl_headmaster,$cl_remarks);
        if ($result) {
             var_dump($result);
           } else {
             echo "操作失败";
